...
tagFile(int fileDescriptor, char* key, char* value, int valueLength)
{
  ...
  keysize = strlen(key);
  if(keysize < 1 || keysize > 9) return -1;
  if(valueLength > 18) return -1;
...
struct Tag*
searchKey(char* key, uchar* bufdata)
{
  struct Tag* curr;
  int i = 0;
  for(curr = (struct Tag*)bufdata; i < 16; curr++){
    if(strncmp(curr->key, (char*)key, strlen(key)) == 0){
      return curr;
    }
    i++;
  }
